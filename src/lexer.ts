...
export function parse(
  ...
  if (!wasm)
    ...
    return init.then(() => parse(source)) as unknown as ReturnType<
      typeof parse
    >;
  ...
  const extraMem =
    ((wasm.__heap_base.value || wasm.__heap_base) as number) +
    len * 4 -
  ...
  (isLE ? copyLE : copyBE)(
    source,
    new Uint16Array(wasm.memory.buffer, addr, len)
  );

  if (!wasm.parse())
    throw Object.assign(
      new Error(
        `Parse error ${name}:${source.slice(0, wasm.e()).split('\n').length}:${
          wasm.e() - source.lastIndexOf('\n', wasm.e() - 1)
        }`
      ),
      { idx: wasm.e() }
    );
  ...
  while (wasm.re()) {
    ...
    const n = source.slice(s, e),
      ch = n[0];
    const ln = ls < 0 ? undefined : source.slice(ls, le),
      lch = ln ? ln[0] : '';
    exports.push({
      s,
      e,
...
export const init = WebAssembly.compile(
  (binary =>
    typeof Buffer !== 'undefined'
      ...
      : Uint8Array.from(atob(binary), x => x.charCodeAt(0)))('WASM_BINARY')
)
